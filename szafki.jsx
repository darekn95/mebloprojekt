...
const defaultCab = {
  ...
  corner: { on: false, arm: 600, doors: "wsporniki" },
...
const assemblyParts = (project, runs) =>
  runs.map((run) => {
    ...
    const mount = Math.max(0, Math.round(run.mountY || 0));
    let x = 0;
...
const armsIn = (full, n, rysowane) => {
  ...
  if (!fn) return [];
  // ramiona lezace w tym pasie — tylko te, ktorych szafka jest na rysunku
  const swoje = fn.arms.filter((a) => rysowane.has(a.run.id));
              ...
              const bok = (u0, w) => w <= 0 ? null : (
                <g key={"bok" + u0}>
                  <rect x={mx(n.ex + u0, w)} y={fy(hTop)} width={w} height={hTop - g.mount}
                    fill={g.cabs[0].mat.board.color} opacity="0.3"
            ...
            {armsIn(full, L.info.get(g.run.id), rysowane).map((a, i) => {
              ...
              return (
                <g key={"arm" + i}>
                  ...
                  {(() => {
                    if (a.doors === "fix") return null;
                    const H = a.cab.cab.H;
                    const y0 = fy(a.cab.base + H);
                    /* Zawias siedzi przy koncu dalszym od naroza; patrzac od
                       tylu caly rysunek jest w lustrze, wiec i on. */
                    const zawiasLewo = rear ? a.outerAtEnd : !a.outerAtEnd;
                    const hx = zawiasLewo ? ax : ax + a.len - HINGE_W;
...
function Assembly3D({ project, runs, open, yaw, pitch, angle, rpOf }) {
  ...
  groups.forEach((g) => {
    ...
    if (rp && rp.h > 0)
      box(0, g.mount, 0, g.total, g.mount + rp.h, g.cabs[0].geo.carcassDepth * 0.9,
        g.cabs[0].mat.board.color, null, 0.9);
    g.cabs.forEach((c) => {
      const cd = c.geo.carcassDepth;
      const t = c.geo.t;
      const bf = c.mat.board.color;
      const y0 = c.base, y1 = c.base + c.cab.H;
      // skorupa korpusu: boki, wieniec, dno, plecy
      box(c.x, y0, 0, c.x + t, y1, cd, bf);
      box(c.x + c.geo.W - t, y0, 0, c.x + c.geo.W, y1, cd, bf);
      box(c.x, y0, 0, c.x + c.geo.W, y0 + t, cd, bf);
      box(c.x, y1 - t, 0, c.x + c.geo.W, y1, cd, bf);
      if (c.cab.back !== "none")
    ...
    n.arms.forEach((a) => {
      ...
      const y0 = c.base, y1 = c.base + c.cab.H;
      box(x0, y0, 0, x0 + a.len, y1, a.depth, c.mat.board.color);
      box(x0, y0, -c.geo.tf, x0 + a.len, y1, 0, c.frontColor, null, 1, true);
      if (c.plinthH > 0)
        box(x0, y0 - c.plinthH, 0, x0 + a.len, y0, a.depth * 0.9, c.mat.board.color, null, 0.9);
    });
  });
  place = null;
  ...
  solids.forEach((sol) => {
    ...
    QUADS.forEach((q) => {
      ...
      faces.push({ pts, depth, color: sol.color,
...
const NoteLine = ({ text, color, icon, editLevels, editItemLevels, cab, setGap, runFix, setMatDepth }) => {
  ...
  const btns = actions.map((action) => {
    ...
    if (action.startsWith("cornercol:")) {
      const [, w, side, idx] = action.split(":");
      return {
        label: `Zrób z tego przejście ${w} mm bez frontu`,
        // uwaga bywa widoczna z innej szafki ciagu, wiec poprawka idzie po numerze
        run: () => editItemLevels(Number(idx), (L) => L.forEach((lv) => {
          const c = newColumn(0, 0);
          c.w = Number(w);
          /* Przejscie do ramienia nie ma przegrody — inaczej zamknelaby wnetrze
             tam, gdzie ma byc przelot do drugiej sciany. */
          if (side === "right") {
            lv.cols[lv.cols.length - 1].noDiv = true;
            lv.cols.push(c);
          } else {
            c.noDiv = true;
            lv.cols.unshift(c);
...
const runPlinth = (project, run) => {
  ...
  if (!(h > 0)) return null;
  return { total, h, ...splitAtJoints(total, joints, run.plinthCuts),
...
const cornerArmParts = (a) => {
  ...
  panels.push({ name: "Wieniec i dno ramienia", qty: 2, a: len, b: a.depth - t, matKey: "board",
  ...
  panels.push({ name: fix ? "Fix ramienia" : "Front ramienia", qty: 1,
    a: Math.max(0, len - (fix ? 0 : 2 * luz)),
    b: d0 ? Math.round(d0.h) : H, matKey: "front",
    edges: { a1: true, a2: true, b1: true, b2: true }, note: "cztery krawędzie" });
  /* Wsporniki trzymaja kat prosty i daja drzwiom miejsce na otwarcie. Przy
     drzwiach lamanych albo skreconych na staly kat nie sa potrzebne. */
  if (a.doors === "wsporniki")
    panels.push({ name: "Wspornik narożnika", qty: 2, a: CORNER_SUPPORT_W, b: inner, matKey: "board",
      edges: { a1: true, a2: true, b1: false, b2: false }, note: "obie krawędzie pionowe" });
...
const projectParts = (project) => {
  ...
  if ((project.runs || []).some((r) => r.corner)) {
    projectLayout(project).info.forEach((n) => {
      ...
      arms.push({ mat: n.arm.cab.rawMat, grainMatters: n.arm.cab.cab.grainMatters,
...
const cornerPairMsgs = (n, blat) => {
  ...
  if (a) {
    ...
    const opisDrzwi = {
      wsporniki: "dwa osobne fronty domykane do dwóch pionowych wsporników w wewnętrznym rogu",
    ...
    if (a.front > 0 && zajete > a.front + 1)
      out.push({ level: "warn", text:
        ...
        + `— nad ramieniem frontu nie ma. Ustaw w tej szafce kolumnę ${fmt(a.front)} mm z drzwiami, `
        + "a resztę szerokości zostaw bez frontu."
        + `|cornercol:${Math.round(a.depth)}:${a.side}:${a.cab.index}` });
    if (a.doors === "wsporniki" && a.front > 0)
      out.push({ level: "info", text:
        `Wsporniki narożnika: 2 szt. po ${fmt(CORNER_SUPPORT_W)} mm szerokości, na całą wysokość wnętrza — `
        + "usztywniają róg i dają frontom miejsce na otwarcie." });
  }
...
export default function App() {
  ...
  const [project, setProjectRaw] = useState({
    ...
    prices: {},
    items: [{ cab: defaultCab, mat: defaultMaterials, runId: null }],
    runs: [],
    active: 0,
  });
  ...
  const groupBar = useMemo(() => {
    ...
    const rows = runs.map((r) => ({ id: r.id, label: r.name }));
  ...
  const scopeOpts = useMemo(() => {
    ...
    if ((project.runs || []).filter((r) => runItems(project, r.id).length).length > 1)
  ...
  const scopeRuns = useMemo(() => {
    if (scope === "run" && runInfo) return [runInfo.run];
    if (scope === "all")
      return (project.runs || []).filter((r) => runItems(project, r.id).length);
    return null;
  }, [scope, project, runInfo]);
                ...
                <>
                  ...
                  <Field label="Wieszanie ciągu"
                    hint={runRl
                      ...
                      : "Ciąg stojący nie potrzebuje zawieszek."}>
                ...
                view === "3d" ? (
                  <div className="space-y-3">
                    <div className="cursor-grab active:cursor-grabbing touch-none"
                      ...
                      <Assembly3D project={project} runs={scopeRuns} open={open3d}
            ...
            <Card title="Uwagi">
              {(errors.length > 0 || warns.length > 0) && (
                <ul className="space-y-2">
                  {errors.map((m, i) => (
                    <NoteLine key={"e" + i} text={m.text} color={ERRC} icon="×" editLevels={editLevels} editItemLevels={editItemLevels} cab={cab} setGap={setGap} runFix={runFix} setMatDepth={setMatDepth} />
                  ))}
                  {warns.map((m, i) => (
                    <NoteLine key={"w" + i} text={m.text} color={WARNC} icon="!" editLevels={editLevels} editItemLevels={editItemLevels} cab={cab} setGap={setGap} runFix={runFix} setMatDepth={setMatDepth} />
                  ))}
                </ul>
              )}
              {infos.length > 0 && (
                <div className={(errors.length || warns.length) ? "border-t border-stone-100 pt-3" : ""}>
                  ...
                  <ul className="space-y-2">
                    {infos.map((m, i) => (
                      <NoteLine key={"i" + i} text={m.text} color="#78716c" icon="i" editLevels={editLevels} editItemLevels={editItemLevels} cab={cab} setGap={setGap} runFix={runFix} setMatDepth={setMatDepth} />
                    ))}
                  </ul>
                </div>
              )}
