  ...
  Object.entries(orders).forEach(([label, cmp]) => {
    ...
    ["h", "v"].forEach((mode) => {
      Object.entries(RECT_RULES).forEach(([ruleName, rule]) => {
        ...
        const cand = { ...res, used, cuts, total, label, mode, ruleName };
        if (!anyRun) anyRun = cand;
        if (res.rejected.length) return;
        const better =
          !best ||
          cand.sheets.length < best.sheets.length ||
          (cand.sheets.length === best.sheets.length && cand.cuts < best.cuts) ||
          (cand.sheets.length === best.sheets.length && cand.cuts === best.cuts && cand.used > best.used);
        if (better) best = cand;
      });
    });
  });
          ...
          {cutPlan && (
            <Card title={`Rozkrój na płycie${cutPlan.scope === "project" ? " — cały projekt" : ""}`}
              ...
              <p className="text-xs text-stone-500">
                ...
                gdzie słoje nie mają znaczenia.
              </p>
