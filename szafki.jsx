...
const makeRun = (runs) => {
  ...
  return { id: nextRunId(runs), name: `Ściana ${Math.max(0, ...nums) + 1}`, wallW: null, gap: 0,
    H: null, D: null, plinth: null, plinthCuts: null, mountY: 0 };
};

// wymiary, ktore w ciagu musza byc wspolne — inaczej fronty i blat sie rozjada
const RUN_SHARED = ["H", "D", "plinth"];
...
function AssemblyView({ project, runs, rpOf, variant, showDims }) {
  ...
  const padT = showDims ? (anyGap ? 210 : 130) : 40;
...
export default function App() {
  ...
  const addCabinet = useCallback((tplId, runId = null) => setProject((p) => {
    ...
    const fresh = { cab: { ...base, name: `Szafka ${next}` }, mat: defaultMaterials, runId };
