...
const TEMPLATES = [
  ...
  },
  {
    ...
    make: () => {
      ...
      /* Z lica korpusu w rogu zostaje tylko to, czego nie zaslania drugi ciag
         (razem z jego frontem), a z tego jeszcze kawalek zabiera katownik
         i luz do niego. */
      const wolne = CORNER_L_W - CORNER_L_D - defaultMaterials.front.thickness;
      /* Nachodzaca plyta katownika stoi po stronie krotszych drzwi, czyli tutaj:
         wchodzi w kwadrat styku obu lic, wiec poza naroze wystaje o grubosc
         frontu mniej niz jej szerokosc. */
      col.doorWidths = [wolne - (CORNER_BRACKET_W - defaultMaterials.front.thickness)
        - defaultCab.gaps.between];
      return {
  ...
  levels.forEach((lv) => {
    ...
    lv.cols.forEach((c, j) => {
      ...
      const licoDo = ctx && ctx.armFree > 0
        ? (ctx.armSide === "right" ? ctx.armFree : W) : W;
      c.rails = rawRails.map((r) => {
        ...
        const cw = c.x1 - c.x0;
        const rx0 = Math.max(c.x0, licoOd);
        const rx1 = Math.min(c.x1, licoDo);
        ...
        return { orient: "front", x0: rx0, x1: rx1, y0: ry1 - rh, y1: ry1, z0, zLen: t, a: rw, b: rh };
      });
...
const armCtxOf = (ref, index) => {
  ...
  ref.info.forEach((k) => {
    if (k.arm && k.arm.cab.index === index)
      out = { armFree: k.arm.free, armSide: k.arm.side };
  });
  return out;
};
            ...
            {[...full.info.values()]
              ...
              .map((k, i) => {
                ...
                const u0 = a.side === "right"
                  ...
                  : c.x + c.geo.W - a.free;
                return (
                  <rect key={"kmask" + i} x={mx(ox + u0, wM)}
            ...
            {armsIn(full, n, rysowane).map((a, i) => (
              <g key={"arm" + i} transform={`translate(${a.u0}, 0)`}>
                ...
                <rect x={a.outerAtEnd ? a.len - a.armFront : 0}
                  ...
                  fill={a.cab.frontColor} stroke={INK} strokeWidth="2" />
                {/* plecy ramienia — bez nich w rzucie zostawala dziura przy scianie */}
                {a.cab.cab.back !== "none" && (
                  <rect x={0} y={0} width={a.len} height={a.cab.geo.tb || a.cab.geo.t}
...
function Assembly3D({ project, runs, open, yaw, pitch, angle, rpOf }) {
  ...
  groups.forEach((g) => {
    ...
    if (rp && rp.h > 0 && g.cabs.length) {
      ...
      box(0, g.mount, cokZ, g.total, g.mount + rp.h, cokZ + cokT,
    ...
    g.cabs.forEach((c) => {
      ...
      const katN = armOf.get(c.index);
      if (katN && katN.bracket && katN.free > 0) {
        const kb = katN.bracket;
        const wM = kb.odKorpusu - kb.luz;
        /* Nachodzaca plyta ma pelne 60 mm i wchodzi w kwadrat styku obu lic —
           rysowana tylko do naroza wygladala, jakby obie dochodzily do siebie
           na styk, a maja sie zachodzic. */
        const pelna = kb.nachodziKorpus ? kb.w : wM;
        const mx0 = katN.side === "right"
          ? katN.free - wM
          : c.geo.W - katN.free - (pelna - wM);
        const shcK = shelfColorOf(c.cab, c.mat);
        if (wM > 0) {
          box(c.x + mx0, y0, -c.geo.tf, c.x + mx0 + pelna, y1, 0, c.frontColor, null, 1, true);
          // za maskownica stoi plyta wewnetrzna katownika, z plyty polkowej
          box(c.x + mx0, y0, 0, c.x + mx0 + pelna, y1, c.geo.t, shcK);
        }
      }
    ...
    n.arms.forEach((a) => {
      ...
      if (c.geo.hasTop) {
        ...
        /* Z przodu plyta na plask, z tylu stojaca — tak samo jak w szafce. Obie
           koncza sie na katowniku, a stojaca cofa sie od plecow o grubosc plyty.
           W ukladzie ramienia z = 0 to LICO, a z = glebokosc plecy. */
        const wLen = a.armFront != null ? a.armFront : a.len;
        // rog jest przy poczatku ramienia, gdy `outerAtEnd` — wzmocnienia zaczynaja
        // sie wtedy za katownikiem, czyli od strony wolnego konca
        const wU0 = x0 + (przyKoncuBok ? a.len - wLen : 0);
        box(wU0, y1 - tA, 0, wU0 + wLen, y1, 100, bA);
        /* Tylne wzmocnienie nie konczy sie na maskownicy jak przednie — biegnie
           przez cale ramie, az do katownika w tylnym narozniku szafki. */
        box(x0, y1 - 100, a.depth - 2 * tA, x0 + a.len, y1, a.depth - tA, bA);
      }
      if (c.cab.back !== "none") {
        const tb = c.geo.tb || tA;
        box(x0, y0, Math.max(0, a.depth - tb), x0 + a.len, y1, a.depth,
      ...
      /* Maskownica katownika stoi w licu drzwi, przy samym rogu, a za nia plyta
         wewnetrzna. Nachodzaca siega pelnych 60 mm i zachodzi na sasiadke. */
      if (kat) {
        /* Formatka maskownicy ma zawsze `kat.w` — to, ktora z plyt nachodzi,
           zmienia tylko miejsce jej konca, nie szerokosc. */
        const mu1 = kat.odRamienia - kat.luz;
        const mu0 = mu1 - kat.w;
        const pelna = kat.w;
        const m0 = przyKoncu ? mu0 : a.len - mu1;
        const shcA = shelfColorOf(c.cab, c.mat);
        box(x0 + m0, y0, -tfA, x0 + m0 + pelna, y1, 0, c.frontColor, null, 1, true);
        box(x0 + m0, y0, 0, x0 + m0 + pelna, y1, c.geo.t, shcA);
      }
    });
  });
  place = null;
...
const NoteLine = ({ text, color, icon, przed, editLevels, editItemLevels, editItemCab, cab, setGap, runFix, setMatDepth }) => {
  ...
  const btns = actions.map((action) => {
    ...
    if (action.startsWith("rundepth:")) {
      const d = Number(action.split(":")[1]);
      return { label: `Zmniejsz głębokość szafek do ${fmt(d)} mm`, run: () => runFix && runFix(action) };
    }
...
const runTop = (project, run) => {
  ...
  return {
    ...
    mat: list.length ? list[0].it.mat : ramiona[0].cab.rawMat,
...
const cornerArmParts = (a) => {
  ...
  if (!armTop)
    panels.push({ name: "Wzmocnienie ramienia", qty: 2,
      a: a.armFront != null ? a.armFront : len, b: 100, matKey: "board",
      edges: { a1: true, a2: false, b1: true, b2: false },
      note: "z przodu na płask, z tyłu stojące — kończą się na kątowniku, zamiast wieńca" });
  panels.push({ name: "Bok ramienia", qty: 1, a: a.depth - t, b: inner, matKey: "board",
    edges: { a1: true, a2: false, b1: false, b2: false }, note: "czoło" });
  if (cab.back !== "none")
    panels.push({ name: "Plecy ramienia", qty: 1, a: len, b: inner, matKey: "back",
      edges: { a1: false, a2: false, b1: false, b2: false }, note: "" });
  ...
  const hardware = fix
    ...
    : [{ name: "Zawias", spec: "front ramienia szafki narożnej", qty: 3, unit: "szt." }];
  if (a.doors === "lamane")
...
const worktopMsgs = (project, run) => {
  ...
  if (rt && rt.worktop && rt.depth > WORKTOP_DEPTHS[0] && rt.depth <= WORKTOP_DEPTHS[1]) {
    ...
    const tf = lvl.pod.length ? lvl.pod[0].geo.tf : 18;
    const glebokosc = lvl.pod.length
...
const cornerPairMsgs = (n, blat) => {
  ...
  if (a) {
    ...
    const fr = (a.cab.geo.doors || []).filter((d) => d.w > 0 && d.type !== "blenda");
    const zajete = fr.length
      ? (a.side === "right"
        ? Math.max(...fr.map((d) => d.x + d.w))
        : a.cab.geo.W - Math.min(...fr.map((d) => d.x)))
      : 0;
    if (a.front > 0 && zajete > a.front + 5)
      out.push({ level: "warn", text:
...
export default function App() {
  ...
  const runFix = useCallback((action) => {
    ...
    if (action.startsWith("rundepth:")) {
      const d = Math.max(1, Math.round(Number(action.split(":")[1]) || 0));
      setRunShared(runInfo.run.id, { D: d });
      return;
    }
            ...
            <Card title="Uwagi">
              {(errors.length > 0 || warns.length > 0) && (
                <ul className="space-y-2">
                  ...
                  {warns.map((m, i) => (
                    <NoteLine key={"w" + i} text={m.text} color={WARNC} icon="!" editLevels={editLevels} editItemLevels={editItemLevels} editItemCab={editItemCab} cab={cab} setGap={setGap} runFix={runFix} setMatDepth={setMatDepth} />
                  ))}
                </ul>
              )}
              {infos.length > 0 && (() => {
                const nowe = infos.filter((m) => !przeczytane.has(m.text));
                ...
                const linia = (m, i, czyt) => (
                  <NoteLine key={(czyt ? "p" : "i") + i} text={m.text} color="#78716c" icon="i"
                    przed={(
                      <button onClick={() => oznaczPrzeczytane(m.text, !czyt)}
