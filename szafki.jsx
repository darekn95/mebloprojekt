...
const DEFAULT_HW_PRICES = {
  ...
  "Wkręt 4 × 30": 0.08,
  /* Zawiasy szafki naroznej ze skrzydlami lamanymi: pierwsze skrzydlo na
...
function computeGeo(cab, mat, ctx) {
  ...
  /* Katalogi okuc, z ktorych liczy ta aplikacja (skrzynki V-BOX, minimalne
     wysokosci frontow, cofniecia wzmocnien), sa podane dla plyty 18 mm. Innej
     nie blokujemy — trzeba tylko wiedziec, ze te liczby wymagaja sprawdzenia. */
  const NORMALNA_PLYTA = 18;
  const inne = [
    t !== NORMALNA_PLYTA ? `korpus ${fmt(t)} mm` : null,
    tf !== NORMALNA_PLYTA ? `front ${fmt(tf)} mm` : null,
    ts !== t && ts !== NORMALNA_PLYTA ? `półki ${fmt(ts)} mm` : null,
  ].filter(Boolean);
  if (inne.length)
    add("warn", `Płyta inna niż 18 mm (${inne.join(", ")}) — wymiary skrzynek szuflad`
      + " i minimalne wysokości frontów liczone są z katalogu dla 18 mm, sprawdź je przed zamówieniem.");
  ...
  /* Szafka narozna ze skrzydlami lamanymi: pierwsze skrzydlo wisi na boku i
     musi otworzyc sie szerzej niz zwykle, zeby drugie zdazylo sie zlozyc —
     stad zawias 165 stopni zamiast zwyklego. Drugie skrzydlo wisi juz na tym
     pierwszym, na zawiasie lamanym, i liczy sie przy ramieniu. */
  const skrzydlaLamane = !!(cab.corner || {}).on && (cab.corner || {}).doors === "lamane";
  if (hingeCount)
    hardware.push({
  ...
  if (cab.plinth && cab.plinth.on) {
    if (cab.legs && cab.legs.on) {
      ...
      /* Bez nozek cokol jest skrecany do korpusu na plastikowe trojkaty.
         To JEDYNE miejsce, gdzie styk dostaje mniej niz dwa laczniki: cokol
         jest mocowany z trzech stron naraz (oba konce plus dlugi bok), wiec
         pojedynczy trojkat na koncu nie ma sie jak obrocic i calosc zostaje
         sztywna. Wzdluz dlugiego boku co ok. 300 mm, nie mniej niz dwa. */
      ...
      hardware.push({
        ...
        spec: `cokół skręcany bez nóżek: po 1 na każdy krótki bok + ${alongLong} wzdłuż ${fmt(plinthLen)} mm (co ok. 300 mm)`,
        qty: 2 + alongLong,
        unit: "szt.",
      });
    }
...
const cornerArmParts = (a) => {
  ...
  /* Fix przykreca sie od srodka na trojkaty meblarskie — tak samo jak cokol
     bez nozek. Trojkatow nie wpisujemy na sztywno: para na kazdym koncu i
     kolejna co ok. 400 mm wysokosci, zeby wysoki fix nie trzymal sie na
     czterech. Zawiasow tyle, ile wychodzi z wysokosci i szerokosci skrzydla —
     tak samo jak przy drzwiach szafki. */
  const trojkatow = 2 * Math.max(2, Math.ceil(inner / 400));
  const zawiasow = autoHinges(d0 ? Math.round(d0.h) : H, armFrontPlan(a).w);
