...
const hwDefaultPrice = (h) => {
  ...
  return DEFAULT_HW_PRICES[h.name] || 0;
};
...
export default function App() {
  ...
  /* Rozkroj grupuje arkusze po samej nazwie materialu, a cena zalezy jeszcze
     od koloru — stad podreczna mapa nazwa → { kolor, rola }. */
  const matByLabel = useMemo(() => {
    const m = new Map();
    project.items.forEach((it) => {
      Object.entries(it.mat || {}).forEach(([key, v]) => {
        if (v && v.name && !m.has(v.name)) m.set(v.name, { color: v.color, key });
      });
    });
    return m;
  }, [project]);
